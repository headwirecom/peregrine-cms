...
<template>
	<div class="wrap">
		<ul
			v-if="schema.listBox && !schema.preview"
			class="check-list"
			:disabled="disabled"
		>
			<li v-for="item in items">
				<span>
					<input
						:id="`material-checklist-${getItemName(item)}`"
						type="checkbox"
						:checked="isItemChecked(item)"
						:disabled="disabled"
						@change="onChanged($event, item)"
					/>
					<label :for="`material-checklist-${getItemName(item)}`">{{
						getItemName(item)
					}}</label>
				</span>
			</li>
		...
		<div
			v-if="!schema.listBox && !schema.preview"
			class="select-wrapper checklist-dropdown"
				...
				<ul class="dropdown-content select-dropdown">
					...
					>
						<span>
							...
							/>
							<label
								:for="`material-checklist-${getItemName(item)}`"
	...
	export default {
		...
		methods: {
			getItemValue(item) {
				if (_.isObject(item)) {
					if (
						typeof this.schema["checklistOptions"] !==
							"undefined" &&
						typeof this.schema["checklistOptions"]["value"] !==
							"undefined"
					) {
			...
			getItemName(item) {
				if (_.isObject(item)) {
					if (
						typeof this.schema["checklistOptions"] !==
							"undefined" &&
						typeof this.schema["checklistOptions"]["name"] !==
							"undefined"
					) {
			...
			isItemChecked(item) {
				return (
					this.value &&
					this.value.indexOf(this.getItemValue(item)) != -1
				);
			},
