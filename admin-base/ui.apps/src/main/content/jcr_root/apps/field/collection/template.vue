	...
	<div class="wrap">
		...
		>
			...
			>
				...
				>
					...
					<span v-if="schema.multifield">{{
						itemName(item, index)
					}}</span>
					<input v-else ref="input" v-model="value[index]" />
					<i
				...
				<transition @enter="enter" @leave="leave" :css="false">
					<div
						v-if="schema.multifield && activeItem === index"
						class="collapsible-body"
					>
						<vue-form-generator
							:schema="schema"
							:model="prepModel(item, schema)"
						></vue-form-generator>
					</div>
				</transition>
			</li>
		...
		<ul v-else class="collection">
			<template v-for="(item, i) in value">
				...
				<li v-else class="collection-item" :key="item + i">
					{{ item }}
				</li>
			</template>
		</ul>
		...
		methods: {
			...
			onRemoveItem(item, index) {
				...
				if (this.schema.multifield) {
					if (item.hasOwnProperty("path")) {
						...
						copy._opDelete = true;
						if (!_deleted[this.schema.model])
							_deleted[this.schema.model] = [];
				...
				if (!this.schema.multifield) {
					...
					if (index === this.activeItem) {
						...
						this.$nextTick(function () {
							...
							this.$nextTick(() => {
								let firstField =
									this.$refs.collapsible.querySelector(
										"li.active input"
									);
			...
			onReorder(oldIndex, newIndex) {
				...
				this.value.splice(
					newIndex,
					0,
					this.value.splice(oldIndex, 1)[0]
				);
				this.$forceUpdate();
			},
