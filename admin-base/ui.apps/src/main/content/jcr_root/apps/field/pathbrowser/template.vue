	...
	<div class="wrap">
		<template v-if="!schema.preview">
			...
			<button
				...
				:disabled="disabled"
				v-on:click.stop.prevent="browse"
				class="btn-flat"
			>
	...
	export default {
		...
		methods: {
			...
			browse() {
				...
				if (!root) {
					console.warn(
						"browserRoot not defined in schema. All paths are available."
					);
					root = "/";
				}
				...
				if (!type) {
					root === `${this.getBasePath()}/pages`
						? (type = PathBrowser.Type.PAGE)
						: (type = PathBrowser.Type.ASSET);
				}
				...
				if (this.isValidPath(selectedPath, root)) {
					currentPath = selectedPath.substr(
						0,
				...
				if (
					options &&
					options.withLink !== null &&
					options.withLink !== undefined
				) {
				...
				$perAdminApp
					.getApi()
					.populateNodesForBrowser(currentPath, "pathBrowser")
					.then((path, name, data) => {
						this.isOpen = true;
					})
					.catch((err) => {
