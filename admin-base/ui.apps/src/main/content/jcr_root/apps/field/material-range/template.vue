	...
	export default {
		...
		watch: {
			value(val) {
				...
				if (isDefined(val)) {
					propsToRemove = propsToRemove.filter(
						(x) => x !== this.schema.model
					);
				} else {
		...
		methods: {
			...
			onRangeValueKeyPress(event) {
				...
				const allowNegative =
					this.min < 0 && event.key === "-" && pos === 0;
				let num = Number(event.key);

				if (
					(!allowNegative && isNaN(num)) ||
					event.key === null ||
					event.key === " "
				) {
					event.preventDefault();
					this.toast.numeric = $perAdminApp.toast(
					...
					if (value < this.min) {
						event.preventDefault();
						this.toast.min = $perAdminApp.toast(
							`Number too low (>= ${this.min})`,
							Toast.Level.INFO
						);
					} else if (value > this.max) {
						event.preventDefault();
						this.toast.max = $perAdminApp.toast(
							`Number too high (<= ${this.max})`,
							Toast.Level.INFO
						);
					}
				}
