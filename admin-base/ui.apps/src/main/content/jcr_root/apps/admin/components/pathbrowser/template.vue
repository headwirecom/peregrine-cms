...
<template>
	<transition name="modal">
		<div
			class="modal-mask pathbrowser-modal-mask"
			v-on:click.stop.prevent="onCancel"
		>
			<div class="modal-wrapper" ref="modalWrapper">
				<div
					class="pathbrowser modal-container"
					ref="pathbrowser"
				...
				>
					...
					<ul class="pathbrowser-tabs">
						<li class="tab">
							<a
								href="#"
								:class="tab === 'browse' ? 'active' : ''"
								v-on:click="select('browse')"
							>
						...
						<li class="tab">
							<a
								href="#"
								:class="tab === 'cards' ? 'active' : ''"
								v-on:click="select('cards')"
							>
					...
					<div
						class="pathbrowser-filter"
						:style="`width: calc(100% - ${searchTabOffset}px)`"
					>
						<input
							placeholder="search"
							type="text"
					...
					<div class="modal-content">
						<div class="col-browse">
							<table
								v-if="search"
								class="highlight pathbrowser-search-results"
							>
								<thead>
									<tr>
										<th></th>
									...
									<div class="modal-content-section">
										<ul class="cards-toolbar sort-nav">
											...
											<li>
												...
												/>
												<label
													v-on:click="onSort('name')"
													for="assetbrowser_sort_cards_name"
													>name</label
												>
											</li>
											<li>
												...
												<label
													v-on:click="
														onSort('resourceType')
													"
													for="assetbrowser_sort_cards_type"
													>type</label
												>
											</li>
											<li>
												...
												<label
													v-on:click="
														onSort('created')
													"
													for="assetbrowser_sort_cards_date"
													>date</label
												>
											</li>
										</ul>
										<ul class="cards-toolbar filter-nav">
											...
											<li>
												...
												<label
													v-on:click="onFilter('*')"
													for="assetbrowser_filter_cards_all"
													>all</label
												>
											</li>
											<li>
												...
												<label
													v-on:click="
														onFilter('files')
													"
													for="assetbrowser_filter_cards_files"
													>files</label
												>
											</li>
											<li>
												...
												<label
													v-on:click="
														onFilter('folders')
													"
													for="assetbrowser_filter_cards_folders"
													>folders</label
												>
											</li>
										</ul>
							...
							<template
								v-if="withLinkTab && tab === 'link' && !search"
							>
								...
								<div
									class="form-group"
									v-if="altText !== undefined"
								>
									<label for="altText"
										>Image Alternate Text</label
									>
									<input
										id="altText"
										type="text"
								...
								<div
									v-if="
										isImageExtension({ path: selectedPath })
									"
									class="img-group"
								>
									<div class="form-group">
								...
								<div class="checkboxes-group">
									<div
										class="pathbrowser-newwindow"
										v-if="newWindow !== undefined"
									...
									>
										...
										/>
										<label for="rel">
											Add rel="noopener noreferrer" to
											link?
										</label>
									</div>
								</div>
					...
					<div class="modal-footer">
						<div class="pathbrowser-footer-details">
							<span class="pathbrowser-selected-path">{{
								selectedPath
							}}</span>
						</div>
						<div class="pathbrowser-buttons">
							...
							<button
								v-on:click="$emit('select', selectedPath)"
								class="
									modal-action
									waves-effect waves-light
									btn-flat
	...
	export default {
		...
		computed: {
			...
			allowFolderSelection() {
				return (
					!this.isBrowserTypeImage &&
					!this.isBrowserTypeObjectDefinition
				);
			},
			isBrowserTypeImage() {
			...
			isBrowserTypeAsset() {
				return (
					this.isType(PathBrowser.Type.ASSET) ||
					this.isType(PathBrowser.Type.IMAGE)
				);
			},
			...
			navigateFolder(item) {
				$perAdminApp
					...
					.then(() => {
						...
						if (this.tab === "cards" && this.list.length > 0)
							this.updateIsotopeLayout("masonry");
