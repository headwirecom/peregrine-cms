			...
			<div class="col s12">
				...
				<ul class="pagination">
					...
					>
						<a
							href=""
							v-bind:disabled="!hasPrevious"
							v-on:click.stop.prevent="loadPage(-1)"
	...
	export default {
		...
		methods: {
			loadPage: function (increment) {
				if (
					(this.hasNext && increment > 0) ||
					(this.hasPrevious && increment < 0)
				) {
			...
			restoreRecyclable(me, target) {
				const heading = `${me.$i18n("Restore")} "${
					target.path
				}" ${me.$i18n("from")} ${target.date_deleted}`;
				$perAdminApp.askUser(
					heading,
					me.$i18n("Are you sure you want to restore this?"),
					{
						yes() {
			...
			deleteRecyclable(me, target) {
				const heading = `${me.$i18n("Delete")} "${
					target.path
				}" ${me.$i18n("from")} ${target.date_deleted}`;
				$perAdminApp.askUser(
					heading,
					me.$i18n("Delete this item forever?"),
					{
						yes() {
