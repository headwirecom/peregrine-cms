		...
		<table>
			<tbody v-if="references">
				...
				<tr>
					<td>
						...
						<span>({{ printStatus(references) }})</span>
					</td>
					<td class="switch">
						<label>
							<input
								type="checkbox"
								v-model="references.publish"
								disabled
							/>
							<span class="lever publishingaction"></span>
						</label>
					</td>
			...
			>
				...
				<tr v-for="ref in references.references" v-bind:key="ref.path">
					<td>
						{{ ref.path }} <span>({{ printStatus(ref) }})</span>
					</td>
					<td class="switch">
						<label>
							<input type="checkbox" v-model="ref.publish" />
							<span class="lever publishingaction"></span>
						</label>
					</td>
			...
			<tbody v-if="referencedBy && referencedBy.length > 0">
				...
				<tr v-for="refed in referencedBy" v-bind:key="refed.path">
					<td>
						{{ refed.path }} <span>({{ printStatus(refed) }})</span>
					</td>
					<td class="switch">
						<label>
							<input type="checkbox" v-model="refed.publish" />
							<span class="lever publishingaction"></span>
						</label>
					</td>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			$perAdminApp
				.getApi()
				.populateReferences(this.path, true)
				.then(function () {
			...
			$perAdminApp
				.getApi()
				.populateReferencedBy(this.path, true)
				.then(function () {
					me.referencedBy = me.trimReferences(
