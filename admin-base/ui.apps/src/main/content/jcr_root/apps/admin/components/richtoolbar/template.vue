<template>
	<div
		class="richtoolbar"
		:class="{ disabled: !inlineRich || preview === 'preview' }"
	>
		<richtoolbar-group
			...
			:collapse="
				!alwaysActiveGroup.noCollapse && alwaysActiveGroup.collapse
			"
		...
		<template v-for="(group, groupIndex) in filteredGroups">
			<richtoolbar-group
				...
				:class="group.class"
				@toggle-click="
					group.toggleClick ? group.toggleClick() : () => {}
				"
				@click="exec($event.btn.cmd)"
			/>
		</template>
		<richtoolbar-group
			...
			:collapse="
				!responsiveMenuGroup.noCollapse && responsiveMenuGroup.collapse
			"
...
<script>
	...
	export default {
		...
		computed: {
			...
			inline() {
				if (!$perAdminApp.getView() || !$perAdminApp.getView().state)
					return null;
			...
			formattingItems() {
				...
				for (let i = 1; i <= 6; i++) {
					headlines.push({
						...
						icon: "title",
						class: () =>
							this.itemIsTag(`H${i}`) ? "active" : null,
		...
		methods: {
			...
			getInlineContainer() {
				...
				return this.getInlineDoc().querySelector(
					".inline-edit.inline-editing"
				);
			},
			execCmd(cmd, value = null, showUi = false) {
				if (!this.getInlineDoc() || !this.getInlineDoc().execCommand)
					return;
			...
			queryCmdState(cmd) {
				if (
					!this.getInlineDoc() ||
					!this.getInlineDoc().queryCommandState
				)
					return;
			...
			insertLink() {
				...
				const text = range.startContainer.textContent.substr(
					start,
					len
				);
			...
			insertIcon(imgPath) {
				...
				this.execCmd(
					"insertHTML",
					`<peregrine-icon img="${imgPath}"></peregrine-icon>`
				);
			},
			...
			itemIsTag(tagName) {
				...
				if (selection) {
					...
					return (
						(start && start.parentNode.tagName === tagName) ||
			...
			startBrowsing() {
				$perAdminApp
					...
					.catch((err) => {
						$perAdminApp
							.getApi()
							.populateNodesForBrowser("/content", "pathBrowser");
					});
			},
			saveSelection() {
				this.selection.buffer = saveSelection(
			...
			onBrowserSelect() {
				...
				this.$nextTick(() => {
					if (["editLink", "insertLink"].includes(this.param.cmd)) {
						...
						["insertImage", "editImage"].includes(this.param.cmd)
					) {
						this.onImageSelect();
					}
			...
			onLinkSelect() {
				if (this.param.cmd === "insertLink") {
					...
					link.setAttribute(
						"target",
						this.browser.newWindow ? "_blank" : "_self"
					);
					link.setAttribute(
						"rel",
						this.browser.rel ? "noopener noreferrer" : ""
					);
					this.restoreSelection();
					this.$nextTick(() => {
			...
			onImageSelect() {
				if (this.param.cmd === "editImage") {
					...
					this.param.value = `<img src="${this.browser.path.selected}"
