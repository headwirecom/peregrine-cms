...
<script>
	...
	export default {
		...
		computed: {
			state() {
				const state = $perAdminApp.getNodeFromView(
					"/state/componentExplorer"
				);
				if (state) {
					return state;
				}
				Vue.set($perAdminApp.getView().state, "componentExplorer", {
					filter: "",
					group: "",
				});
			...
			isIgnoreContainersEnabled() {
				...
				return (
					...
					view.state.tools.workspace.ignoreContainers ===
		...
		methods: {
			...
			isActive(key, groupChildren) {
				return this.state.accordion[key];
			},
				...
				if (ev) {
					if (component.variation) {
						ev.dataTransfer.setData(
							"text",
							component.path + ":" + component.variation
						);
					} else {
