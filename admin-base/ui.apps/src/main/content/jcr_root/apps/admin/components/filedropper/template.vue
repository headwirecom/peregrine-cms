...
<script>
	...
	export default {
		...
		mounted() {
			this.$nextTick(() => {
				//ensure that this.dropContext element is present
				this.dropContext.addEventListener(
					"dragenter",
					this.onDropContextDragEnter
				);
			});
		},
		beforeDestroy() {
			this.dropContext.removeEventListener(
				"dragenter",
				this.onDropContextDragEnter
			);
		},
		methods: {
			...
			upload(files) {
				...
				$perAdminApp
					...
					.then((uploadedFiles) => {
						return $api
							.populateNodesForBrowser(this.path, "pathBrowser")
							.then(() => uploadedFiles);
					})
					.then((uploadedFiles) => {
