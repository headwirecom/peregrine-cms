	...
	<transition name="modal">
		<div class="modal-mask" v-on:click.stop.prevent="onCancel">
			<div class="modal-wrapper">
				<div
					class="pathbrowser iconbrowser modal-container"
					v-on:click.stop.prevent="onPrevent"
				>
								...
								>
									<div class="item-content">
										<i
											...
											v-bind:style="`font-size: ${cardIconSize(
												cardSize
											)}px`"
											>{{ icon.text }}</i
										>
					...
					<div class="modal-footer">
						<span class="selected-path">
							<i
								v-if="currentIcon"
								v-bind:class="currentIcon.class"
								>{{ currentIcon.text }}</i
							>
							{{
								currentIcon.family === "material"
									? currentIcon.text
									: currentIcon.class
							}}
						</span>
						...
						<button
							...
							class="
								...
								btn-flat
							"
							v-on:click.prevent="onSelect(currentIcon)"
						>
							Select
						</button>
					</div>
		...
		mounted() {
			if (this.selectedIcon) {
				...
				if (this.selectedIcon.family) {
					...
					this.selectedFamily.value = this.camelize(
						this.selectedIcon.family
					);
				}
			}
		},
		...
		computed: {
			icons() {
				...
				return peregrineFontFamilies
					.map((icon) => icon)
					.filter(
						(icon) => icon.family === this.selectedFamily.value
					);
			},
			iconFamilies() {
