	...
	>
		...
		<div class="tenant-collection">
			<template v-if="children && children.length > 0">
				<div
					class="col s12 m6 l6 icon-action m-left-inherit"
					v-for="child in children"
					v-bind:key="child.name"
				>
					<div class="card blue-grey darken-3">
						<div
							class="card-content white-text tenant-link"
							@click="onCardContentClick(child.name)"
						>
...
<script>
	export default {
		...
		computed: {
			children: function () {
				const tenants = $perAdminApp.getNodeFrom(
					$perAdminApp.getView(),
					"/admin/tenants"
				);
				if (tenants) {
		...
		methods: {
			selectTenant(vm, target) {
				$perAdminApp
					.stateAction("setTenant", { name: target.name })
					.then(() => {
						$perAdminApp.loadContent(
							`/content/admin/pages/welcome.html/path:/content/${target.name}`
						);
					});
			},
			...
			onCreateNewSiteClick() {
				$perAdminApp.action(
					this,
					"selectPath",
					"/content/admin/pages/pages/createtenant"
				);
			},
