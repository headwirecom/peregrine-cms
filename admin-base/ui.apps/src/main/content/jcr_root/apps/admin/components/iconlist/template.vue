...
<template>
	<div class="row">
		<admin-components-iconaction
			v-for="action in fromSource"
			v-bind:key="action.path"
			v-bind:model="toTenant(action)"
		></admin-components-iconaction>
	</div>
</template>
		...
		methods: {
			toTenant: function (action) {
				...
				if (res.action.startsWith("/content/admin/pages/pages")) {
					...
					res.action.startsWith("/content/admin/pages/assets")
				) {
					res.action += `.html/path:/content/${tenant.name}/assets`;
				} else if (
					res.action.startsWith("/content/admin/pages/objects")
				) {
					res.action += `.html/path:/content/${tenant.name}/objects`;
				} else if (
					res.action.startsWith("/content/admin/pages/templates")
				) {
					res.action += `.html/path:/content/${tenant.name}/templates`;
				} else if (
					res.action.startsWith(
						"/content/admin/pages/object-definitions"
					)
				) {
					res.action += `.html/path:/content/${tenant.name}/object-definitions`;
				} else if (
					res.action.startsWith("/content/admin/pages/recyclebin")
				) {
