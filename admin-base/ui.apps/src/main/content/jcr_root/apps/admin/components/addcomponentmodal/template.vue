	...
	<div v-show="visible" class="add-component-modal-wrapper">
		<div class="add-component-modal">
			<div ref="header" class="header">
				Add Component: <b>{{ computedDrop }}</b>
			</div>
			<div class="content">
				<input
					ref="filter"
...
<script>
	...
	export default {
		...
		computed: {
			...
			filteredComponents() {
				return get(this.view, "/admin/components/data", []).filter(
					(el) => {
						const tenant = this.view.state.tenant;
						const lCmpName =
							this.componentDisplayName(el).toLowerCase();
						...
						if (
							...
							!tenant.name ||
							el.group === ".hidden" ||
							!lCmpName.startsWith(filter)
						) {
							return false;
						} else {
							return el.path.startsWith(
								"/apps/" + tenant.name + "/"
							);
						}
					}
		...
		methods: {
			...
			addComponentFromModal() {
				...
				$perAdminApp
					...
					.then(() => {
						return $perAdminApp.stateAction(
							"addComponentToPath",
							payload
						);
					})
			...
			findNewNode(payload) {
				if (["after", "before"].includes(payload.drop)) {
					...
					const parentNode = $perAdminApp.findNodeFromPath(
						this.view.pageView.page,
						parentPath
					);
					let index = null;
			...
			clearWindowEventListeners() {
				this.windowEventListeners.forEach((listener) => {
					listener.window.removeEventListener(
						listener.type,
						listener.id
					);
				});
