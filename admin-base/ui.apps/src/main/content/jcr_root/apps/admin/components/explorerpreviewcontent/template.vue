	...
	<div :class="['explorer-preview-content', `preview-${nodeType}`]">
		<template v-if="currentObject">
			...
			<template v-else-if="isTab(Tab.REFERENCES)">
				...
				<ul
					...
					:class="[
						'collection',
						'with-header',
						`explorer-${nodeType}-referenced-by`,
					]"
				>
					<li class="collection-header">
						referenced in {{ referencedBy.length }} location<span
							v-if="referencedBy.length !== 1"
							>s</span
						>
					</li>
					<li
						v-for="item in referencedBy"
						:key="item.path"
			...
			<template v-else-if="isTab(Tab.PUBLISHING)">
				<span class="panel-title">{{ getActiveTabName }}</span>
				<admin-components-publishinginfo
					v-bind:node="nodeFromPath"
					v-if="nodeFromPath"
				/>
				...
				<div v-if="allowOperations && node" class="action-list">
					<div
						class="action"
						:title="`Open Web Publishing ${nodeType} Dialog`"
						@click="openPublishingModal()"
					>
						<i class="material-icons">publish</i>
						Publish to Web ({{ nodeType }})
					</div>
					<div
						v-if="nodeFromPath.activated && !isReferencedInPublish"
						class="action"
						:title="`Deactivate ${nodeType}`"
					>
						<admin-components-action
							:model="{
								target: node.path,
								command: 'unPublishResource',
								tooltipTitle: `${$i18n('undo publish')} '${
					...
					<div
						v-else
						class="action operationDisabledOnActivatedItem"
						:title="`Deactivate ${nodeType}`"
					>
						<span>
							<i class="material-icons">remove_circle_outline</i>
			...
			<template v-else-if="isTab(Tab.ACTIONS)">
				...
				<div v-if="allowOperations" class="action-list">
					...
					<div
						:class="classForActionDisabledOnActivatedResource"
...
<script>
	...
	export default {
		...
		computed: {
			...
			allowMove() {
				return (
					this.nodeTypeGroups.allowMove.indexOf(this.nodeType) > -1
				);
			},
			hasOgTags() {
			...
			hasReferences() {
				return (
					this.nodeTypeGroups.references.indexOf(this.nodeType) > -1
				);
			},
			hasMultipleTabs() {
			...
			referencedBy() {
				if ($perAdminApp.getView().state.referencedBy) {
					return this.trimReferences(
						$perAdminApp.getView().state.referencedBy.referencedBy
					);
				}
				return [];
			},
			versions() {
				return this.hasVersions
					? $perAdminApp.getView().state.versions.versions
					: [];
			},
			isImage() {
			...
			hasVersions() {
				return $perAdminApp.getView().state.versions
					? $perAdminApp.getView().state.versions.has_versions
					: false;
			},
			nodeName() {
			...
			classForActionDisabledOnActivatedResource() {
				return this.selfOrAnyDescendantActivated
					? "action operationDisabledOnActivatedItem"
					: "action";
			},
			stateToolsEdit() {
				const stateTools =
					$perAdminApp.getNodeFromViewOrNull("/state/tools");
				if (stateTools) {
		...
		methods: {
			itemToTarget(path) {
				...
				if (path.startsWith(`/content/${tenant.name}/pages`)) {
					...
					if (path.startsWith(`/content/${tenant.name}/assets`)) {
						ret.load =
							ret.path = `/content/admin/pages/assets.html/path:${path}`;
						ret.type = "ASSET";
					} else if (
						path.startsWith(`/content/${tenant.name}/objects`)
					) {
						ret.load =
							ret.path = `/content/admin/pages/objects.html/path:${path}`;
						ret.type = "OBJECT";
					}
				}
				return ret;
			...
			getSchema(schemaKey) {
				...
				const cmpDefinition =
					view.admin.componentDefinitions[component];
				if (!cmpDefinition) {
					return {};
				}
			...
			getObjectComponent() {
				...
				if (!resourceType) {
					resourceType =
						this.rawCurrentObject.data["sling:resourceType"];
				}
			...
			performRenameNode(newName, newTitle) {
				const that = this;
				$perAdminApp
					.stateAction(`rename${this.uNodeType}`, {
					...
					.then(() => {
						if (
							that.nodeType === "asset" ||
							that.nodeType === "object"
						) {
							const currNode = $perAdminApp.getNodeFromView(
								`/state/tools/${that.nodeType}/show`
							);
							const currNodeArr = currNode.split("/");
							currNodeArr[currNodeArr.length - 1] = newName;
							$perAdminApp.getNodeFromView(
								`/state/tools/${that.nodeType}`
							).show = currNodeArr.join("/");
						} else {
							// page and template handling
							const currNode =
								$perAdminApp.getNodeFromView("/state/tools")[
									that.nodeType
								];
							const currNodeArr = currNode.split("/");
							currNodeArr[currNodeArr.length - 1] = newName;
			...
			checkActivationStatusAndPerform(action) {
				if (this.selfOrAnyDescendantActivated) {
					$perAdminApp.toast(
						"You cannot perform this operation yet. The resource or one of its children is still published." +
							" Please unpublish all of them first.",
						"warn",
						7500
					);
				} else {
			...
			copyNode() {
				$perAdminApp
					.getApi()
					.populateNodesForBrowser(this.path.current, "pathBrowser")
					.then(() => {
			...
			deleteNode() {
				this.checkActivationStatusAndPerform(() => {
					const me = this;
					this.onDelete(this.nodeType, this.node.path)
						.then(() => {
							$perAdminApp.stateAction(
								`unselect${me.uNodeType}`,
								{}
							);
						})
						.then(() => {
							...
							$perAdminApp.loadContent(
								"/content/admin/pages/pages.html/path" +
									SUFFIX_PARAM_SEPARATOR +
									path
							);
							me.isOpen = false;
			...
			deleteVersion(me, target) {
				$perAdminApp.stateAction("deleteVersion", {
					path: target.path,
			...
			checkoutVersion(version) {
				if (version.base === true) {
					$perAdminApp.notifyUser(
						"Info",
						"You cannot checkout the current version"
					);
					return;
				}
				let self = this;
			...
			save() {
				...
				if (this.nodeType === NodeType.OBJECT) {
					...
					promise = $perAdminApp.stateAction(
						`save${this.uNodeType}Properties`,
						this.node
					);
					this.edit = false;
				}
			...
			saveObject() {
				...
				let _deleted = $perAdminApp.getNodeFromViewWithDefault(
					"/state/tools/_deleted",
					{}
				);
				...
				for (const key in data) {
					...
					if (
						...
						(Array.isArray(_deleted[key]) &&
							_deleted[key].length &&
							typeof _deleted[key][0] === "object")
					) {
						let node = data[key];
				...
				set(
					$perAdminApp.getView(),
					"/state/tools/save/confirmed",
					true
				);
				const result = $perAdminApp
					.stateAction("saveObjectEdit", { data: data, path: show })
					.then(() => {
			...
			setActiveTab(clickedTab) {
				...
				$perAdminApp.action(this, "setActiveTabName", {
			...
			openLiveVersion() {
				...
				const { primaryDomain, pagePath } = page;
