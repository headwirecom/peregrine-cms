...
<template>
	<nav
		...
		v-bind:class="isExtended ? 'nav-extended' : ''"
	>
		<div class="nav-wrapper blue-grey darken-3">
			<div class="nav-left">
				<div
					class="brand-logo"
					@click="$refs.notificationsModal.open()"
				>
			...
			<div class="nav-center">
				<ul v-if="!hideTenants" class="hide-on-med-and-down nav-mobile">
					<admin-components-action
						...
						:class="{
							active: getActiveSection() === section.name,
				...
				<ul v-else class="hide-on-med-and-down nav-mobile">
					<admin-components-action
						tag="li"
						:model="
							getSectionModel({ title: 'Home', name: 'index' })
						"
			...
			<ul class="nav-right hide-on-med-and-down nav-mobile">
				<admin-components-materializemodal ref="languageModal">
					...
					<template slot="footer">
						<button
							...
							@click="$refs.languageModal.close()"
						>
							{{ $i18n("Close") }}
						</button>
					</template>
				...
				>
					...
					<template slot="header">
						<div
							class="user-circle big"
							:title="$i18n('profile picture')"
						>
...
<script>
	...
	export default {
		...
		computed: {
			hideTenants() {
				return this.model.hideTenants
					? true
					: $perAdminApp.getView().state.tenant
					? false
					: true;
			},
			language() {
			...
			tenantDdItems() {
				return [
					...
					},
					{
						...
						icon: "swap_horiz",
						click: this.onChangeWebsiteClick,
					},
				];
			},
			userDdItems() {
		...
		methods: {
					...
					if (section.name !== "welcome") {
						...
						target +=
							path && path.length > 0
								? `/path:${path}`
								: `/path:${
										this.state.tenant.roots[section.name]
								  }`;
					} else {
			...
			onTutorialsClick() {
				document
					.getElementById("peregrine-main")
					.classList.toggle("tutorial-visible");
			},
			onAboutClick() {
			...
			onSettingsClick() {
				$perAdminApp.action(this, "configureTenant", {
					path:
						"/content/admin/pages/tenants/configure.html/content/" +
			...
			onLogoutClick() {
				window.location.href =
					"/system/sling/logout?resource=/index.html";
			},
			onLanguageClick() {
			...
			getActiveSection() {
				const breadcrumbs =
					$perAdminApp.getView().adminPage.breadcrumbs;
				if (breadcrumbs) {
			...
			copyUsername() {
				...
				if (success) {
					$perAdminApp.toast(
						`copied username <i>"${this.username}"</i>`,
						Toast.Level.INFO
					);
				} else {
					$perAdminApp.toast(
