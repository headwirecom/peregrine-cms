...
<template>
	...
	>
		<div class="explorer-layout">
			<div class="row">
				<div v-if="pt" class="col s12 m8 explorer-main">
					<ul class="collection">
						...
						>
							...
							<admin-components-action
								...
								v-bind:model="{
									...
									tooltipTitle: `${$i18n('edit')} '${
										child.title || child.name
									}'`,
								}"
							>
								<icon v-bind="nodeTypeToIcon(child)" />
							...
							<div class="secondary-content">
								...
								<admin-components-action
									v-if="composumEditable(child)"
									v-bind:model="{
										target: child.path,
										command: 'editFile',
										tooltipTitle: `${$i18n('editFile')} '${
											child.title || child.name
										}'`,
									}"
								>
								...
								<admin-components-action
									v-if="replicable(child)"
									v-bind:model="{
										target: child.path,
										command: 'replicate',
										tooltipTitle: `${$i18n('replicate')} '${
											child.title || child.name
										}'`,
									}"
								>
									<i
										class="material-icons"
										v-bind:class="replicatedClass(child)"
										>public</i
									>
								</admin-components-action>
						...
						>
							...
							>
								<i class="material-icons">add_circle</i>
								{{ $i18n("add object definition") }}
							</admin-components-action>
						</li>
						<li
							class="collection-item"
							v-if="isInsideObjectDefinition(path)"
						>
							<admin-components-action
								v-bind:model="{
									target: '',
					...
					<div
						...
						class="empty-explorer"
					>
						<div v-if="path.includes('assets')">
							{{ $i18n("emptyExplorerHintAssets") }}.
						</div>
						<div v-else>{{ $i18n("emptyExplorerHint") }}...</div>
					</div>
				</div>
				...
				<admin-components-explorerpreview v-if="hasEdit">
					<component
						v-bind:is="model.children[0].component"
						v-bind:model="model.children[0]"
		...
		<admin-components-publishingmodal
			...
			:modalTitle="`Web Publishing: ${publishDialogPath
				.split('/')
				.pop()}`"
	...
	export default {
		...
		computed: {
			...
			path: function () {
				...
				var node = $perAdminApp.getNodeFrom(
					$perAdminApp.getView(),
					dataFrom
				);
				return node;
			...
			children: function () {
				if (this.pt.children) {
					return this.pt.children.filter((child) =>
						this.checkIfAllowed(child)
					);
				}
			},
			parentPath: function () {
		...
		methods: {
			getTenant() {
				return (
					$perAdminApp.getView().state.tenant || { name: "example" }
				);
			},

			isAssets(path) {
				return path.startsWith(
					`/content/${this.getTenant().name}/assets`
				);
			},

			isPages(path) {
				return path.startsWith(
					`/content/${this.getTenant().name}/pages`
				);
			},

			isObjects(path) {
				return path.startsWith(
					`/content/${this.getTenant().name}/objects`
				);
			},

			isObjectDefinitions(path) {
				return (
					!this.isInsideObjectDefinition(path) &&
					path.startsWith(
						`/content/${this.getTenant().name}/object-definitions`
					)
				);
			},
			...
			onDropRow(item, ev, type) {
				if (this.isDraggingUiEl) {
					...
					const path = $perAdminApp.getNodeFrom(
						$perAdminApp.getView(),
						dataFrom
					);
					let action;
					switch (true) {
						...
						default:
							console.warn(
								"path is not a site, asset, template or object."
							);
					}
			...
			isSelected: function (child) {
				if (this.model.selectionFrom && child) {
					return (
						$perAdminApp.getNodeFromViewOrNull(
							this.model.selectionFrom
						) === child.path
					);
				} else if (
					child.path ===
					$perAdminApp.getNodeFromViewOrNull("/state/tools/page")
				) {
					return true;
				}
				return false;
			...
			editable: function (child) {
				return (
					["per:Page", "per:Object"].indexOf(child.resourceType) >= 0
				);
			},
			...
			viewable: function (child) {
				return (
					["per:Page", "per:Object", "nt:file"].indexOf(
						child.resourceType
					) >= 0
				);
			},
			...
			nodeTypeToIcon: function (item) {
				if (item.resourceType === "per:Page")
					return { icon: "description", lib: IconLib.MATERIAL_ICONS };
				if (item.resourceType === "per:Object")
					return { icon: "layers", lib: IconLib.MATERIAL_ICONS };
				if (item.resourceType === "per:ObjectDefinition")
					return {
						icon: "insert_drive_file",
						lib: IconLib.MATERIAL_ICONS,
					};
				if (item.resourceType === "nt:file")
					return this.fileExtToIcon(item);
				if (item.resourceType === "per:Asset")
					return { icon: "image", lib: IconLib.MATERIAL_ICONS };
				if (item.resourceType === "sling:Folder")
					return { icon: "folder", lib: IconLib.MATERIAL_ICONS };
				if (item.resourceType === "sling:OrderedFolder")
					return { icon: "folder", lib: IconLib.MATERIAL_ICONS };
			...
			checkIfAllowed: function (node) {
				if (
					...
					this.model.showFilter === "true" &&
					this.filter
				) {
					if (
						node.excludeFromSitemap &&
						node.excludeFromSitemap === "true"
					)
						return false;
					return (
						[
			...
			showInfo: function (me, target) {
				...
				if (target.startsWith(`/content/${tenant.name}/objects`)) {
					...
					$perAdminApp.stateAction("selectObject", {
						selected: node.path,
					...
					$perAdminApp.stateAction("showTemplateInfo", {
						selected: target,
					});
				} else {
					$perAdminApp.stateAction("showPageInfo", {
						selected: target,
					});
				}
			},
			...
			selectPath: function (me, target) {
				...
				if (resourceType) {
					...
					if (resourceType === "per:Asset") {
						$perAdminApp.stateAction("selectAsset", {
							selected: target.path,
						});
						return;
				...
				$perAdminApp
					.stateAction("selectToolsNodesPath", payload)
					.then(() => {
			...
			addObject: function (me, target) {
				...
				if (path.startsWith(`/content/${tenant.name}/objects`)) {
					$perAdminApp.stateAction("createObjectWizard", {
						path: path,
						target: target,
					});
				}
			...
			addObjectDefinition: function (me, target) {
				...
				if (
					...
					$perAdminApp.stateAction("createObjectDefinitionWizard", {
						path: path,
						target: target,
					});
				}
			...
			addObjectDefinitionFile(me, target) {
				...
				if (this.isInsideObjectDefinition(path)) {
					$perAdminApp.stateAction(
						"createObjectDefinitionFileWizard",
						{ path, target }
					);
				}
			},
			...
			deleteTenantOrPage: function (me, target) {
				if (target.activated) {
					$perAdminApp.toast(
						"The resource is still published. Please unpublish it first.",
						"warn",
						7500
					);
				} else if (target.anyDescendantActivated) {
					$perAdminApp.toast(
						"One of the children of this resource is still published. Please unpublish all of them first.",
						"warn",
						7500
					);
				} else if (target.isReferenced) {
					$perAdminApp.toast(
						"The resource is referenced somewhere. Please remove the references first.",
						"warn",
						7500
					);
			...
			deleteTenant: function (me, target) {
				$perAdminApp.askUser(
					"Delete Site",
					me.$i18n(
						"Are you sure you want to delete this site, its children, and generated content and components?"
					),
					{
						yes() {
