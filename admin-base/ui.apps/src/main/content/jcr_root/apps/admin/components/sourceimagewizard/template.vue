...
<template>
	<div
		...
		v-bind:class="[
			'sourceimagewizard',
			'container',
			{ 'initial-search': !state.results },
		]"
	>
		<div class="search-bar">
			<button
				v-if="viewing"
				v-on:click.prevent.stop="deSelect()"
				class="back-to-grid btn-flat"
			>
			...
			<form
				...
				:class="['image-search', { previewing: viewing }]"
			>
				<input
					type="text"
					v-model="state.input"
					v-bind:placeholder="$i18n('searchImageAsset')"
					tabindex="1"
					autofocus
				/>
				<button
		...
		<div v-if="state.results" class="search-content">
			<span v-if="state.results.length < 1 && !loading" class="no-results"
				>No images found for '{{ state.input }}'</span
			>
			...
			<div v-else-if="viewing" class="image-preview">
				<div class="image-row">
					<button
						v-on:click.prevent.stop="select(viewing.index - 1)"
						:class="[
							'btn-flat',
							'btn-large',
					...
					<div class="image-container">
						...
						<form
							...
							v-on:submit.prevent="
								addImage(
									state.results[viewing.index],
									viewing.name
								)
							"
							class="image-rename-form"
						>
							<input
								type="text"
								v-model="viewing.name"
								autofocus
							/>
							<button
								type="submit"
					...
					<button
						...
						:class="[
							...
							{
								disabled:
									viewing.index == state.results.length - 1,
							},
						]"
					>
				...
				<div class="image-preview-details">
					<span class="resolution"
						>{{ viewing.webformatWidth }} x
						{{ viewing.webformatHeight }}</span
					>
...
<script>
	export default {
		...
		computed: {
			tags() {
				return this.viewing.tags.split(", ");
			},
		},
		...
		methods: {
			requestImages() {
				...
				$.getJSON(URL, (data) => {
					...
					this.state.numPages = Math.ceil(
						data.totalHits / this.itemsPerPage
					);
					this.loading = false;
				});
			},
			...
			addImage(item, name) {
				...
				$perAdminApp.stateAction("fetchExternalAsset", {
					...
					config: {
						onUploadProgress: (ev) =>
							this.uploadProgress(
								Math.floor((ev.loaded * 100) / ev.total)
							),
					},
