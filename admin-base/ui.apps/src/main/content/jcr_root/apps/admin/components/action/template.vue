	...
	<component :is="tag" v-if="visible">
		...
		>
			<i
				...
				v-bind:class="isSelected ? 'actionSelected' : ''"
			>
	...
	export default {
		...
		computed: {
			...
			isSelected() {
				...
				let currentState = $perAdminApp.getNodeFromViewOrNull(
					this.model.stateFrom
				);
				if (!currentState) {
			...
			target() {
				if (
					this.model.target &&
					typeof this.model.target === "string"
				) {
		...
		methods: {
			...
			dblClickAction: function (e) {
				$perAdminApp.action(
					this,
					this.model.dblClickCommand,
					this.model.dblClickTarget
				);
			},
			onClick: function (e) {
