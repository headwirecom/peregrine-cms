...
<script>
	...
	export default {
		...
		computed: {
			...
			targetInline() {
				return (
					this.target.getAttribute(Attribute.INLINE) ||
					this.target
						.querySelector(`[${Attribute.INLINE}]`)
						.getAttribute(Attribute.INLINE)
				);
			},
			view() {
			...
			isSelected() {
				return (
					this.component && this.path && this.path !== "/jcr:content"
				);
			},
			pagePath() {
			...
			previewMode() {
				const ws = $perAdminApp.getNodeFromViewOrNull(
					"/state/tools/workspace"
				);
				return ws ? ws.preview : "";
			},
			viewMode() {
				const viewMode = $perAdminApp.getNodeFromViewOrNull(
					"/state/tools/workspace/view"
				);
				const previewMode = $perAdminApp.getNodeFromViewOrNull(
					"/state/tools/workspace/preview"
				);
				let ret;
				if (viewMode) {
			...
			isTemplateNode() {
				return (
					$perAdminApp.findNodeFromPath(this.pageView.page, this.path)
						.fromTemplate === true
				);
			},
			isRich() {
			...
			componentTitle() {
				const componentName = this.view.state.editor.component
					.split("-")
					.join("/");
			...
			componentIsDropTarget() {
				...
				const dropTargetElements =
					this.component.querySelectorAll(selector);
			...
			sibling() {
				...
				return {
					previous: this.findComponentEl(
						this.component.previousElementSibling
					),
					next: this.findComponentEl(
						this.component.nextElementSibling
					),
				};
			},
		},
			...
			"iframe.dimension": {
				...
				handler() {
					if (this.target) {
						...
						this.wrapEditableAroundElement(
							this.iframe.mouseOverCmp
						);
					}
				},
			},
		...
		mounted() {
			...
			vm.$nextTick(() => {
				...
				if (this.previewMode !== "preview") {
					set(
						$perAdminApp.getView(),
						"/state/contentview/editor/active",
						true
					);
				} else {
					set(
		...
		methods: {
					...
					if (!vm.dragging) {
						if (vm.component !== vm.previousComponent) {
							...
							if (
								vm.autoSave &&
								vm.node &&
								vm.view.state.editor.path
							) {
								vm.autoSave = false;
								$perAdminApp
									.stateAction("savePageEdit", {
										data: vm.node,
			...
			updateSelectedComponent() {
				...
				$perAdminApp
					.action(this, "showComponentEdit", this.path)
					.then(() => {
			...
			findComponentEl(targetEl) {
				...
				let el = targetEl;
				while (
					!el.getAttribute(Attribute.PATH) ||
					el.getAttribute(Attribute.DROPTARGET)
				) {
					el = el.parentElement;
					if (!el) {
						break;
			...
			onInlineEdit(event) {
				if (!this.inlineEdit.firstTime.includes(event.target)) {
					...
					this.inlineEdit.selection = saveSelection(
						event.target,
						this.iframe.doc
					);
				}
				...
				if (!eventPath) {
					this.toast.missingEventPath = $perAdminApp.toast(
						Error.MISSING_EVENT_PATH,
						Toast.Level.ERROR
					);
			...
			onInlineDelete(event) {
				...
				if (
					...
					selection.anchorNode === this.target
				) {
					event.preventDefault();
			...
			onInlineArrowKey(event, isKeyUp = false) {
				...
				if (
					...
					(isKeyUp || this.holdingDown)
				) {
					this.caret.counter++;
					if (this.caret.counter < 2) return;
					const inlineEditNodes = this.iframe.app.querySelectorAll(
						`[${Attribute.INLINE}]`
					);
					...
					for (let i = 0; i < len; i++) {
						if (inlineEditNodes[i] === this.target) {
							if (
								i > 0 &&
								(key === Key.ARROW_LEFT || key === Key.ARROW_UP)
							) {
			...
			onIframeLoaded(ev) {
				...
				this.iframe.app =
					this.iframe.doc.querySelector("#peregrine-app");
				this.iframe.win.addEventListener(
					"resize",
			...
			onIframeDragOver(event) {
				...
				if (this.component) {
					if (!this.dropTarget && this.isTemplateNode) {
						if (
							!(
								(this.isFromTemplate(previousSibling) ||
			...
			onIframeDrop(event) {
				...
				if (componentPath.includes("/components/")) {
					...
					const targetNode = $perAdminApp.findNodeFromPath(
						this.view.pageView.page,
						this.path
					);
			...
			iframeEditMode() {
				...
				this.iframe.doc.addEventListener(
					"dragover",
					this.onIframeDragOver
				);
				this.iframe.doc.addEventListener("drop", this.onIframeDrop);
				this.iframe.doc.addEventListener(
					"mouseover",
					this.onIframeMouseOver
				);
				this.iframe.html.classList.add("edit-mode");
				const elements = this.iframe.app.querySelectorAll(
					`[${Attribute.INLINE}]`
				);
			...
			iframePreviewMode() {
				...
				try {
					this.iframe.doc.removeEventListener(
						"click",
						this.onIframeClick
					);
					this.iframe.doc.removeEventListener(
					...
					console.debug(
						"no event listener to be removed from iframe",
						err
					);
				}
				if (this.iframe.body) {
			...
			addIframeExtraStyles() {
				if (this.iframe.head.querySelector("#editing-extra-styles"))
					return;
				const css = `
          html.edit-mode body {
          ...
          html.edit-mode #peregrine-app .inline-edit {
			...
			getBoundingClientRect(e) {
				...
				const marginTop = parseFloat(
					this.getElementStyle(e, "margin-top")
				);
				const marginLeft = parseFloat(
					this.getElementStyle(e, "margin-left")
				);
				const marginRight = parseFloat(
					this.getElementStyle(e, "margin-right")
				);
				const marginBottom = parseFloat(
					this.getElementStyle(e, "margin-bottom")
				);
				const newRect = {
			...
			getRelativeMousePosition(event) {
				...
				return {
					...
					"y%":
						((event.pageY - offset.top - this.scrollTop) /
							offset.height) *
						100,
				};
			},
    ...
    ============================================ */
			...
			onDelete(e) {
				...
				if (payload.path !== "/jcr:content") {
					$perAdminApp
						.stateAction("deletePageNode", payload)
						.then((data) => {
			...
			onPaste(e) {
				...
				$perAdminApp
					.stateAction("addComponentToPath", payload)
					.then((data) => {
						this.refreshIframeElements();
					});
			},
			...
			onAddComponentModalComponentAdded(newNode) {
				...
				const firstInlineEditEl = newNodeEl.querySelector(
					`[${Attribute.INLINE}]`
				);
			...
			scrollIntoViewCenter(el, doc, win) {
				...
				win.scrollBy(
					0,
					(el.getBoundingClientRect().height - viewportH) / 2
				);
			},
			...
			updateIframeDimensions() {
				...
				this.iframe.timeout = setTimeout(() => {
					this.iframe.dimension.w =
						this.iframe.doc.documentElement.clientWidth;
					this.iframe.dimension.h =
						this.iframe.doc.documentElement.clientHeight;
				}, this.iframe.delay);
			},
		},
	};
