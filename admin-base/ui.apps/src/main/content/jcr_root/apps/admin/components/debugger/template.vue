...
<template>
	<div v-bind:class="`debugger ${elementStyle}`">
		<a
			href="#"
			v-if="!visible"
			v-on:click.stop.prevent="showDebugger(true)"
			v-bind:title="$i18n('showDebugData')"
			class="toggle-debugger show-debugger"
			v-bind:style="consoleErrors"
			><i class="material-icons">bug_report</i></a
		>
		<a
			href="#"
			v-if="visible"
			v-on:click.stop.prevent="showDebugger(false)"
			v-bind:title="$i18n('hideDebugData')"
			class="toggle-debugger hide-debugger"
		...
		<div v-if="visible" class="debugger-content">
			<div class="row">
				<div class="col s12 m4 l3 debugger-levels">
					...
					<ul class="collection">
						<li
							v-for="(logger, key) of getLoggers()"
							class="collection-item right-align"
						>
							<span class="logger-name">{{ logger.name }}:</span>
							<a
								class="logger-level"
								v-bind:title="`${$i18n('set')} ${
									logger.name
								} ${$i18n('loggingLevel')}: ${levelToName(
									logger.level
								)}`"
								v-on:click.stop.prevent="
									changeLogLevel(logger.name)
								"
							>
					...
					<ul class="list-inline">
						<li v-for="(value, key) of this.$root.$data">
							<a
								...
								v-bind:class="selected === key ? 'active' : ''"
								v-on:click.stop.prevent="select(key)"
								>{{ key }}</a
							>
						</li>
					</ul>
					<code>
						<pre>{{ this.$root.$data[this.selected] }}</pre>
					</code>
				</div>
			</div>
		</div>
	...
	export default {
		...
		computed: {
			...
			consoleErrors: function () {
				return $perAdminApp.getView().admin.consoleErrors === true
		...
		methods: {
			...
			getLoggers: function () {
				...
				for (var key in loggers) {
					ret.push({
						name: loggers[key].name,
						level: loggers[key].level,
					});
				}
				return ret;
			},
