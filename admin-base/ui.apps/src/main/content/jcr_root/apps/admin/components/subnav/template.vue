				...
				<template slot="content" v-if="filteredChildren.length > 0">
					<admin-components-nodetreeitem
						...
						:supported-resource-types="
							NodeTree.SUPPORTED_RESOURCE_TYPES
						"
						@click.native.stop="() => {}"
	...
	export default {
		...
		computed: {
			...
			isEditPage() {
				return (
					this.model.classes &&
					this.model.classes.indexOf("navcenter") >= 0
				);
			},
			tenant() {
			...
			showNodeTree() {
				return (
					this.isEditPage &&
					["pages", "templates"].indexOf(this.section) >= 0
				);
			},
			renderRichToolbar() {
				return (
			...
			filteredChildren() {
				if (this.nodeTreeRootNode && this.nodeTreeRootNode.children) {
					return this.nodeTreeRootNode.children.filter((ch) => {
						return this.isSupportedNodeTreeResourceType(
							ch.resourceType
						);
					});
				} else {
		...
		methods: {
			...
			isSupportedNodeTreeResourceType(resourceType) {
				return (
					resourceType &&
					NodeTree.SUPPORTED_RESOURCE_TYPES.indexOf(resourceType) >= 0
				);
			},
			isRich() {
