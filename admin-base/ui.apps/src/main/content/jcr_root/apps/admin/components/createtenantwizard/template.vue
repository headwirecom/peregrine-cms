	...
	<div class="container">
		...
		>
			<tab-content title="select theme" :before-change="leaveTabOne">
				<p>
					This wizard allows you to create a website from an existing
				...
				<fieldset class="vue-form-generator">
					<div class="form-group required">
						...
						<ul class="collection">
							<li
								...
								v-on:click.stop.prevent="
									selectTheme(null, item.name)
								"
								v-bind:class="
									isSelected(item.name) ? 'active' : ''
								"
							>
						...
						<div
							v-if="formErrors.unselectedThemeError"
							class="errors"
						>
			...
			<tab-content title="verify">
				...
				<p>
					We are ready to create the Site
					<b>{{ formmodel.name }}</b> with the site name
					<b>{{ formmodel.title }}</b> from the theme
					<b>{{ formmodel.templatePath }}</b> for you.
				</p>
				<p
					v-if="
						formmodel.colorPalette &&
						formmodel.colorPalette.length > 0
					"
				>
					The website will use the color palette
					<b>{{ colorPaletteName(formmodel.colorPalette) }}</b
					>.
				</p>
				<p>
					Click the <b>Back</b> button to change your choice. Click
					<b>Create and Edit!</b> to create and start editing the home
					page of your website or click <b>Create</b> to create the
					website and go to the dashboard.
				</p>
			</tab-content>
	...
	export default {
					...
					fields: [
						{
							...
							onChanged: (model, newVal, oldVal, field) => {
								if (!this.nameChanged) {
									model.name = $perAdminApp.normalizeString(
										newVal,
										"_"
									);
						...
						},
						{
							...
							model: "tenantUserPwd",
							visible:
								$perAdminApp.getNodeFromView("/state/user") ===
								"admin",
							required: false,
							onChanged: (model, newVal, oldVal, field) => {},
						},
		...
		methods: {
			...
			selectTheme: function (me, target) {
				...
				$perAdminApp
					.getApi()
					.getPalettes(me.formmodel.templatePath)
					.then((data) => {
			...
			onComplete: function (edit = true) {
				...
				if (
					this.formmodel.colorPalette &&
					this.formmodel.colorPalette.length > 0
				) {
			...
			validateTabOne: function (me) {
				me.formErrors.unselectedThemeError =
			...
			validSiteName(value) {
				...
				if (value.match(/[^0-9a-z_]/)) {
					return [
						"site names may only contain lowercase letters, numbers, and underscores",
					];
				}
				return [];
			},
