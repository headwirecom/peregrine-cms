...
<template>
	<div v-bind:data-per-path="model.path">
		...
		>
			...
			></div>
			<div
				class="__pcms_tour_highlite"
				ref="highlite"
				v-bind:style="highliteStyle"
			></div>
			<div
				:class="[
					{ 'no-transition': noTransition },
			...
			>
				<button v-on:click="enabled = false" class="btn-flat btn-close">
					<i class="material-icons">close</i>
				</button>
				<div ref="tourText" v-html="text" class="card-content"></div>
				<div class="card-action">
					<button v-on:click="onPrevious" class="btn">
						{{ $i18n("prev") }}
					</button>
					<button v-on:click="onNext" class="btn">
						{{ $i18n("next") }}
					</button>
				</div>
			</div>
...
<script>
	export default {
		...
		computed: {
			edit() {
				return (
					...
					window.parent.$perAdminApp !== undefined
				);
			},
			bottom() {
			...
			bottomStyle() {
				return {
					...
					left: "0px",
					width: "100%",
					height: `${this.windowHeight - this.bottom}px`,
				};
			},
			topStyle() {
				return {
			...
			infoStyle() {
				const placeLeft = {
					left: `${this.left - this.info.width - 20}px`,
				};
				const placeRight = { left: `${this.right + 10}px` };
				const placeAbove = {
					top: `${this.top - this.info.height - 20}px`,
				};
				...
				if (anchor) {
					let horizontal = {
						left:
							this.left + this.info.width > window.innerWidth
								? `${window.innerWidth - this.info.width}px`
								: `${this.left}px`,
					};
					let vertical = {
						top:
							this.top + this.info.height > window.innerHeight
								? `${window.innerHeight - this.info.height}px`
								: `${this.top}px`,
					};
					...
					if (anchor.indexOf("bottom") > -1)
						vertical = {
							top: `${window.innerHeight - this.info.height}px`,
						};
					if (anchor.indexOf("left") > -1) horizontal = { left: 0 };
					if (anchor.indexOf("right") > -1)
						horizontal = {
							left: `${window.innerWidth - this.info.width}px`,
						};
				...
				const horizontalStyle =
					spaceLeft > spaceRight ? placeLeft : placeRight;
				const verticalStyle =
					spaceAbove > spaceBelow ? placeAbove : placeBelow;

				if (
					spaceBelow > this.info.height + 40 ||
					spaceAbove > this.info.height + 40
				) {
				...
				if (
					spaceLeft > this.info.width + 40 ||
					spaceRight > this.info.width + 40
				) {
					const secondaryStyle = {
						top:
							this.top + this.info.height > window.innerHeight
		...
		methods: {
			...
			showTourItem() {
				const root = this.findElement(
					this.$root,
					this.model.children[this.index].locator
				);
				const el = this.model.children[this.index].selector
					? root.querySelector(
							this.model.children[this.index].selector
					  )
					: root;
				if (el !== null) {
			...
			onPrevious() {
				...
				this.index--;
				if (this.index === -1)
					this.index = this.model.children.length - 1;
				this.showTourItem();
			},
			windowChange() {
				if (this.enabled) {
		...
		updated() {
			...
			this.info.height = this.$refs.info
...
<style>
	...
	.tour-left {
		transition: top 0.25s, width 0.25s, height 0.25s;
	}
	.tour-right {
