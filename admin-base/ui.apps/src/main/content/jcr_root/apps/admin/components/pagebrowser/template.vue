...
<template>
	<div
		id="pageBrowserModal"
		class="pathbrowser pagebrowser modal default modal-fixed-footer"
	>
				...
				<template v-if="tab === 'browse' && !search">
					...
					<ul class="browse-list">
						...
						>
							...
							/>
							<label
								v-on:click.stop.prevent="selectItem(item.path)"
							></label>
						...
						<ul class="cards-toolbar sort-nav">
							...
							<li>
								...
								<label
									...
									for="pagebrowser_sort_cards_name"
									>name</label
								>
							</li>
							<li>
								...
								<label
									...
									for="pagebrowser_sort_cards_date"
									>date</label
								>
							</li>
						</ul>
		...
		computed: {
			selectedPath() {
				return $perAdminApp.getNodeFromViewOrNull(
					"/state/pagebrowser/selectedPath"
				);
			},
			withLinkTab() {
				return $perAdminApp.getNodeFromViewOrNull(
					"/state/pagebrowser/withLinkTab"
				);
			},
		...
		methods: {
			...
			selectFolder(item) {
				$perAdminApp
					...
					.then(() => {
						let pb =
							$perAdminApp.getNodeFromView("/state/pagebrowser");
						pb.root = item.path;
						if (this.tab === "cards") {
							this.updatedIsotopeLayout("masonry");
						}
			...
			selectItem(path) {
				return ($perAdminApp.getNodeFromView(
					"/state/pagebrowser"
				).selectedPath = path);
			},
			onUnlink() {
