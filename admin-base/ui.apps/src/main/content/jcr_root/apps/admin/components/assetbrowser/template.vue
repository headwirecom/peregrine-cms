...
<template>
	<div
		id="assetBrowserModal"
		class="pathbrowser assetbrowser modal default modal-fixed-footer"
	>
					...
					<template v-if="list.length > 0">
						<ul class="cards-toolbar sort-nav">
							...
							<li>
								...
								/>
								<label
									v-on:click="onSort('resourceType')"
		...
		computed: {
			selectedPath() {
				return $perAdminApp.getNodeFromViewOrNull(
					"/state/assetbrowser/selectedPath"
				);
			},
			path() {
				let root = $perAdminApp.getNodeFromViewOrNull(
					"/state/assetbrowser/root"
				);
				return root;
			},
			nodes() {
				...
				if (nodes && this.path) {
					let nodesFromPath = $perAdminApp.findNodeFromPath(
						nodes,
						this.path
					);
		...
		methods: {
			...
			selectFolder(item) {
				$perAdminApp
					...
					.then(() => {
						let pb = $perAdminApp.getNodeFromView(
							"/state/assetbrowser"
						);
						pb.root = item.path;
						if (this.tab === "cards") {
