	...
	<div>
		<div>
			...
			<p>
				...
				is exposed at as well as yout js/css includes, prefetch domains
				and your brand slug
			</p>
		</div>
		<div>
			...
			<p>
				Use the 'site setup replication' button to go live with your
				whole site and push all the necessary information out to your
				web server. Once you're live you can easily push any new pages
				or page changes out in the admin console.
			</p>
		</div>
	...
	export default {
		...
		methods: {
			editRootTemplate(me, target) {
				...
				$perAdminApp.stateAction(
					"editTemplate",
					`/content/${tenant}/templates`
				);
			},
			tenantSetupReplication(me, target) {
				const tenant = $perAdminApp.getView().state.tenant.name;
				$perAdminApp.stateAction(
					"tenantSetupReplication",
					`/content/${tenant}`,
					"true"
				);
			},
			...
			uploadFile(files) {
				const tenant = $perAdminApp.getView().state.tenant.name;
				$perAdminApp.stateAction("uploadBackupTenant", {
					path: `/content/${tenant}`,
					files: files,
		...
		computed: {
			...
			downloadUrl() {
				...
				return tenant
					? "/perapi/admin/downloadBackupTenant.zip/content/" +
							tenant.name
					: "";
			},
			backupInfo() {
