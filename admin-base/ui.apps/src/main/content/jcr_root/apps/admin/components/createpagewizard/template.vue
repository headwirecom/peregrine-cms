	...
	<div class="container">
		...
		>
						...
						<div class="row">
							<div class="col s6">
								<div
									v-if="
										skeletonPages &&
										skeletonPages.length > 0
									"
								>
									<label class="as-heading"
										>Select a Skeleton-Page</label
									>
								...
								>
									<li
										...
										v-on:click.stop.prevent="
											selectTemplate(null, template.path)
										"
										v-bind:class="
											isSelected(template.path)
												? 'active'
												: ''
										"
								...
								<div
									v-if="formErrors.unselectedTemplateError"
									class="errors"
								>
									<span track-by="index"
							...
							<div class="col s6">
								...
								<p>
									The area below shows a preview of the
									selected page type.
								</p>
								<div class="iframe-container">
									<iframe
										v-if="formmodel.skeletonPagePath"
										v-bind:src="
											formmodel.skeletonPagePath + '.html'
										"
										data-per-mode="preview"
									>
									</iframe>
									<iframe
										v-if="formmodel.templatePath"
										v-bind:src="
											formmodel.templatePath + '.html'
										"
										data-per-mode="preview"
									>
									</iframe>
			...
			<tab-content title="verify">
				...
				<span v-if="formmodel.templatePath"
					>template `{{ formmodel.templatePath }}`</span
				>
				<span v-else-if="formmodel.skeletonPagePath"
					>skeleton page `{{ formmodel.skeletonPagePath }}`</span
				>
			</tab-content>
...
<script>
	...
	export default {
		...
		computed: {
			pageSchema() {
				if (this.formmodel.templatePath !== "") {
					...
					if (definitions) {
						...
						const def = $perAdminApp.getNodeFromView(
							"/admin/componentDefinitions"
						)[comp];
						return def;
					}
			...
			templates() {
				const templates = $perAdminApp.getNodeFromViewOrNull(
					"/admin/templates/data"
				);
				...
				return templates.filter((item) =>
					item.path.startsWith(siteRoot)
				);
			},
			skeletonPages() {
				const siteRoot =
					this.formmodel.path.split("/").slice(0, 4).join("/") +
					"/skeleton-pages";
				const skeletonPageRoot = $perAdminApp.findNodeFromPath(
					this.$root.$data.admin.skeletonNodes,
					siteRoot
				);
				if (skeletonPageRoot) {
		...
		methods: {
			...
			isSelected(target) {
				return (
					this.formmodel.templatePath === target ||
					this.formmodel.skeletonPagePath === target
				);
			},
			...
			validateTabOne(me) {
				me.formErrors.unselectedTemplateError =
					"" === "" + me.formmodel.templatePath &&
					"" === "" + me.formmodel.skeletonPagePath;
