...
function bringUpEditor(me, view, target) {
		...
		return new Promise((resolve, reject) => {
			...
			if (name !== "savePageEdit" && name !== "deletePageNode") {
				if (current === view.state.editor.checksum) {
					...
					$perAdminApp.askUser(
						"Save Page Edit?",
						"Would you like to save your page edits?",
						{
							yesText: "Save",
							noText: "Cancel",
							yes() {
	...
	return new Promise((resolve, reject) => {
		return me
			.getApi()
			.populateComponentDefinitionFromNode(view.pageView.path + target)
			.then((name) => {
				...
				set(
					view,
					"/state/editor/checksum",
					JSON.stringify(view.pageView.page, jsonEqualizer, 2)
				);
				resolve();
			})
			.catch((error) => {
				...
				$perAdminApp.notifyUser(
					"error",
					"was not able to bring up editor for the selected component"
				);
				reject();
			});
	});
...
export default function (me, target) {
	...
	return new Promise((resolve, reject) => {
		if (view.state.editorVisible) {
			me.getApi()
				.populatePageView(view.pageView.path)
				.then(() => {
