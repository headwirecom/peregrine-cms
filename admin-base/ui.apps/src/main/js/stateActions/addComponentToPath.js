...
export default function (me, target) {
	...
	const variationSeparator = target.component
		? target.component.indexOf(":")
		: -1;
	const componentPath =
		variationSeparator === -1
			? target.component
			: target.component.slice(0, variationSeparator);
	let variation =
		variationSeparator === -1
			? undefined
			: target.component.slice(
					variationSeparator + 1,
	...
	log.fine(
		"load",
		componentName,
		"into edit view (make sure it is available)"
	);
	document
		.getElementById("editview")
	...
	if (target.drop.startsWith("into")) {
		...
		targetNodeUpdate = me.findNodeFromPath(
			view.pageView.page,
			path.parentPath
		);
		targetNode = me.findNodeFromPath(view.pageView.page, target.path);
		...
		log.error(
			"addComponentToPath() target.drop not in allowed values - value was",
			target.drop
		);
	}
	...
	if (targetNode && target.component) {
		...
		return me
			.getApi()
			.insertNodeAt(
				target.pagePath + targetNode.path,
				componentPath,
				target.drop,
				variation
			)
			.then((data) => {
				if (view.pageView.page.serverSide) {
					...
					return me
						.getApi()
						.populatePageView(me.getNodeFromView("/pageView/path"));
				...
				if (!targetNodeUpdate) {
					...
					if (targetNodeUpdate.fromTemplate === true) {
						return me
							.getApi()
							.populatePageView(
								me.getNodeFromView("/pageView/path")
							);
					} else {
						if (target.drop.startsWith("into")) {
							Vue.set(
