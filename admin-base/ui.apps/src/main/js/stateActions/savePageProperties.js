...
export default function (me, target) {
	...
	for (let i = 0; i < schema.fields.length; i++) {
		if (!schema.fields[i].readonly) {
			...
			const dstName = schema.fields[i]["x-model"]
				? schema.fields[i]["x-model"]
				: srcName;
	...
	for (let i = 0; i < ogTagSchema.fields.length; i++) {
		if (!ogTagSchema.fields[i].readonly) {
			...
			const dstName = ogTagSchema.fields[i]["x-model"]
				? ogTagSchema.fields[i]["x-model"]
				: srcName;
			nodeData[dstName] = target[srcName];
		}
	}
	...
	return new Promise((resolve, reject) => {
		me.getApi()
			.savePageEdit(target.path, nodeData)
			.then(() => {
				resolve();
			});
	});
