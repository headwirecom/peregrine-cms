...
function update(path) {
	...
	return axios
		.post(API_BASE + path, null, postConfig)
		.then((response) => {
...
function updateWithForm(path, data) {
	...
	return axios
		.post(API_BASE + path, data, postConfig)
...
function updateWithFormAndConfig(path, data, config) {
	...
	logger.fine(
		"Update with Form and Config, path: " + path + ", data: " + data
	);
...
function fetchRef(service, path, sameTenant = false) {
	return fetch(
		`/admin/${service}.json${path}?${new URLSearchParams({ sameTenant })}`
	);
}
...
class PerAdminImpl {
	...
	populateToolsConfig() {
		return fetch("/admin/list.json/tools/config").then((data) =>
			populateView("/admin", "toolsConfig", data.children)
		);
	}

	populateUser() {
		return fetch("/admin/access.json?" + new Date().getTime()).then(
			(data) => {
	...
	populateContent(path) {
		return fetch("/admin/content.json" + path).then((data) =>
			populateView("/", "adminPageStaged", data)
		);
	}

	populateComponents() {
		return fetch("/admin/components.json").then((data) =>
			populateView("/admin", "components", data)
		);
	}

	populateObjects() {
		return fetch("/admin/objects.json").then((data) =>
			populateView("/admin", "objects", data)
		);
	}

	populateTemplates() {
		return fetch("/admin/templates.json").then((data) =>
			populateView("/admin", "templates", data)
		);
	}

	populateSkeletonPages(
		path,
		target = "skeletonNodes",
		includeParents = false
	) {
		const skeletonPagePath =
			path.split("/").slice(0, 4).join("/") + "/skeleton-pages";
		...
		return this.populateNodesForBrowser(
			skeletonPagePath,
			target,
			includeParents
		);
	...
	populateComponentDefinitionFor(component) {
		return fetch("/admin/components/" + component).then((data) =>
			populateView("/admin/componentDefinitions", component, data)
		);
	}
	...
	populateComponentDefinitionFromNode(path) {
		return new Promise((resolve, reject) => {
			var name;
			fetch("/admin/componentDefinition.json" + path).then((data) => {
				name = data.name;
				...
				if (
					...
					data.model = component.methods.augmentEditorSchema(
						data.model
					);
					data.ogTags = component.methods.augmentEditorSchema(
						data.ogTags
					);
				}
				...
				if (data && data.model) {
					if (data.model.groups) {
						for (let j = 0; j < data.model.groups.length; j++) {
							for (
								let i = 0;
								i < data.model.groups[j].fields.length;
								i++
							) {
								let from =
								...
								let visible =
									data.model.groups[j].fields[i].visible;
								if (visible) {
									data.model.groups[j].fields[i].visible =
										function (model) {
											return exprEval.Parser.evaluate(
												visible,
												this
											);
										};
						...
						for (let i = 0; i < data.model.fields.length; i++) {
							...
							if (visible) {
								data.model.fields[i].visible = function (
									model
								) {
									return exprEval.Parser.evaluate(
										visible,
						...
						if (data.ogTags) {
							for (
								...
								if (visible) {
									data.ogTags.fields[i].visible = function (
										ogTags
									) {
										return exprEval.Parser.evaluate(
											visible,
	...
	populateTenants() {
		return new Promise((resolve, reject) => {
			fetch("/admin/listTenants.json").then((data) => {
	...
	populateBackupInfo(backup) {
		...
		if (tenantName === "" || tenantName === "undefined") {
			const tenant = $perAdminApp.getNodeFromViewWithDefault(
				"/state/tenant",
				{}
			);
			tenantName = tenant ? tenant.name : "";
		}
		fetch("/admin/backupTenant.json/content/" + tenantName).then((data) =>
			populateView("/state/tools", "backup", data)
		);
	}

	populatePageView(path) {
		return fetch("/admin/readNode.json" + path).then((data) =>
			populateView("/pageView", "page", data)
		);
	}

	populateObject(path, target, name) {
		return this.populateComponentDefinitionFromNode(path).then(() => {
			return fetch("/admin/getObject.json" + path).then((data) =>
	...
	populateReferencedBy(path, sameTenant = false) {
		return fetchRef("refBy", path, sameTenant).then((data) =>
		...
		return new Promise((resolve, reject) => {
			fetchRef("ref", path, sameTenant)
				.then(function (result) {
					populateView("/state", "references", result).then(() =>
						resolve()
					);
				})
				.catch((error) => {
	...
	populateI18N(language) {
		return new Promise((resolve, reject) => {
			axios
				.get("/i18n/admin/" + language + ".infinity.json")
				.then((response) => {
	...
	populateRecyclebin(page = 0) {
		...
		return new Promise((resolve, reject) => {
			fetch(`/admin/listRecyclables.json/content/${tenant}?page=${page}`)
				.then(function (result) {
					populateView("/admin", "recyclebin", result).then(() =>
						resolve()
					);
				})
				.catch((error) => {
		...
		if (page) {
			return new Promise((resolve, reject) => {
				fetch(`/admin/listVersions.json${page}`)
					.then(function (result) {
						populateView("/state", "versions", result).then(() =>
							resolve()
						);
					})
					.catch((error) => {
	...
	recycleItem(item) {
		return new Promise((resolve, reject) => {
			...
			updateWithForm(
				"/admin/restoreRecyclable.json" + item.recyclebinItemPath,
				data
			)
						...
						callbacks
							...
							.then(function () {
								...
								if (pagesRgx.test(path)) {
									nodes =
										$perAdminApp.getView().state.tools
											.pages;
	...
	createObject(parentPath, name, templatePath) {
		...
		data.append("name", name);
		data.append("templatePath", templatePath);
		return updateWithForm(
			"/admin/createObject.json" + parentPath,
			data
	...
	deleteObject(path) {
		let data = new FormData();
		return updateWithForm("/admin/deleteNode.json" + path, data).then(() =>
	...
	moveAsset(path, to, type) {
		...
		return updateWithForm("/admin/move.json" + path, data).then(() =>
			this.populateNodesForBrowser(path)
		);
	}
	...
	deletePage(path) {
		...
		return updateWithForm("/admin/deletePage.json" + path, data).then(() =>
			this.populateNodesForBrowser(path)
		);
	}
	...
	deleteTenant(target) {
		...
		const data = new FormData();
		data.append("name", name);
		return updateWithForm("/admin/deleteTenant.json", data).then(() =>
			this.populateNodesForBrowser(root)
		);
	}
	...
	deletePageNode(path, nodePath) {
		let data = new FormData();
		return updateWithForm("/admin/deleteNode.json" + nodePath, data).then(
			() => this.populatePageView(path)
		);
	}
	...
	createTemplate(parentPath, name, component, title) {
		...
		data.append("title", title);
		return updateWithForm(
			"/admin/createTemplate.json" + parentPath,
			data
		).then(() => this.populateNodesForBrowser(parentPath));
	}
	...
	createObjectDefinition(parentPath, name) {
		...
		data.append("name", name);
		return updateWithForm(
			"/admin/createObjectDefinition.json" + parentPath,
			data
		).then(() => this.populateNodesForBrowser(parentPath));
	}
	...
	createFolder(parentPath, name) {
		...
		data.append("name", name);
		return updateWithForm(
			"/admin/createFolder.json" + parentPath,
			data
		).then(() => this.populateNodesForBrowser(parentPath));
	}
		...
		return this.populateNodesForBrowser(path)
			...
			.then((addedFiles) => {
				if (addedFiles.length > 0) {
					...
					return updateWithFormAndConfig(uri, formData, config).then(
						(data) => ({ addedFiles, data })
					);
				}
			})
		...
		return new Promise((resolve, reject) => {
			...
			try {
				...
				if (type === "pages") {
					...
					callbacks.getView().state.tools.templates =
						pagePath.join("/");
				}
				return $perAdminApp
					.stateAction("showPageInfo", { selected: page })
					.then(() => {
						resolve();
					});
			} catch (error) {
				logger.error(
					"setting of path in initial page editor state failed"
				);
				logger.error(error);
				reject();
			}
	...
	savePageEdit(path, node) {
		return new Promise((resolve, reject) => {
			...
			if (nodeData.component) {
				let component = callbacks.getComponentByName(
					nodeData.component
				);
				if (
					component &&
					component.methods &&
					component.methods.beforeSave
				) {
			...
			if (node.component) {
				nodeData["sling:resourceType"] = node.component
					.split("-")
					.join("/");
			}
			stripNulls(nodeData);
			...
			updateWithForm(
				"/admin/updateResource.json" + path + node.path,
				formData
			)
				// .then( (data) => this.populateNodesForBrowser(parentPath) )
				.then(() => resolve())
	...
	saveAssetProperties(node) {
		return new Promise((resolve, reject) => {
			...
			formData.append("content", json(nodeData));
			updateWithForm(
				"/admin/updateResource.json" + node.path + "/jcr:content",
				formData
			).then(() => resolve());
		});
	}
				...
				.then((respData) => {
					...
					noticeFunction = setInterval(function () {
						function stopPolling(data) {
							const lastAction =
								data["per:ReplicationLastAction"];
						...
						return fetch(
							...
							if (count++ >= 25) {
								...
								$perAdminApp
									.getApi()
									.populateNodesForBrowser(parentPath);
								$perAdminApp.notifyUser(
									"Success",
									`${data.sourcePath} was successfully ${
										deactivate ? "un" : ""
									}published.`
								);
							}
	...
	getPalettes(templateName) {
		return fetch(
			`/admin/nodes.json/content/${templateName}/pages/css/palettes`
		)
			.then((data) => {
			...
			.catch((err) => {
				logger.warn(
					`template ${templateName} does not support palettes`
				);
			});
	}
	...
	populateIcons(tenant) {
		return fetch(`/admin/nodes.json/content/${tenant.name}/assets/icons`)
			.then((data) => {
				const iconsNode = $perAdminApp.findNodeFromPath(
					data,
				...
				logger.debug(
					`populated icons for tenant ${tenant.name}:`,
					icons
				);
			})
			.catch((err) => {
	...
	tenantSetupReplication(path, withSite) {
		...
		return updateWithForm(
			"/admin/tenantSetupReplication.json" + path,
			formData
		);
	}
	...
	uploadBackupTenant(path, files, cb) {
		...
		if (!data.entries().next().done) {
			return updateWithFormAndConfig(
				"/admin/uploadBackupTenant.json" + path,
				data,
				config
			)
				.then(() => this.populateNodesForBrowser(path))
	...
	acceptTermsAndConditions() {
		let formData = new FormData();
		return updateWithForm(
			"/admin/acceptTermsAndConditions.json",
			formData
		).then(() => {
