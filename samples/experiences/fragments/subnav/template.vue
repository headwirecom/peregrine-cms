<template>
	<div v-bind:data-per-path="model.path">
		...
		<select
			v-model="experiences.currentComponent"
			style="display: inline; width: 200px; color: black"
		>
			<option value="" disabled>Choose a component</option>
			<option
				v-for="component in components"
				v-bind:value="component.path"
			>
				{{ component.name }}
			</option>
		</select>
		<select
			v-model="experiences.currentExperience"
			style="display: inline; width: 200px; color: black"
		>
			<option value="">none</option>
			<option v-for="item in experienceList" v-bind:value="item">
				{{ item }}
			</option>
		</select>
	</div>
	...
	export default {
		...
		beforeMount() {
			...
			this.$watch("experiences.currentExperience", function (newVal) {
				if (newVal) {
					Vue.set(
						$perAdminApp.getNodeFromView("/state"),
						"currentExperiences",
						[newVal]
					);
				}
		...
		computed: {
			components() {
				let ret = [];
				const components = $perAdminApp.getNodeFromView(
