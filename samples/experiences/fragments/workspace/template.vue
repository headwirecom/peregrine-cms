	...
	<div v-bind:data-per-path="model.path">
				...
				<li v-for="item in componentList">
					...
					<div class="row">
						<div class="col m12 s12">
							{{ item.path }} {{ listExperiences(item) }}
						</div>
						<div class="col m6 s12">
							<experiences-components-reference
								v-bind:model="{ ref: item.path }"
							></experiences-components-reference>
						</div>
						<div class="col m6 s12">
							{{ modelFor(item).path }}
							<vue-form-generator
								v-bind:model="modelFor(item)"
								v-bind:schema="schemaFor(item)"
								v-bind:options="{
		...
		<div v-if="editing">
			<admin-components-editor
				v-bind:model="{ component: 'admin-components-editor' }"
			></admin-components-editor>
		</div>
	</div>
</template>
...
<script>
	export default {
		...
		data() {
			return {
				experiences: $perAdminApp.getNodeFromView("/state/experiences"),
			};
		},
		methods: {
			schemaFor(item) {
				return {
					fields: [{ type: "input", model: "text" }],
				};
			},
			modelFor(item) {
				const experiences = $perAdminApp.getNodeFromView(
					"/state/currentExperiences"
				);
				if (experiences) {
					if (item.experiences && item.experiences.children) {
						for (
							let i = 0;
							i < item.experiences.children.length;
							i++
						) {
							const expNode = item.experiences.children[i];
							if (
								expNode.experiences.join(",") ===
								experiences.join(",")
							) {
								return expNode;
							}
		...
		computed: {
			layoutView() {
				...
				return (
					$perAdminApp.getNodeFromView("/pageView/path") +
					".html" +
					"#" +
					experiences
				);
			...
			componentList() {
				const component = this.componentPathToName(
					this.experiences.currentComponent
				);
				const page = $perAdminApp.findNodeFromPath(
					$perAdminApp.getView().pageView.page,
					"/jcr:content/components"
				);
				let ret = [];
