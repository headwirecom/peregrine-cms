<template>
	<themeclean-components-block v-bind:model="model" @mounted="onBlockMounted">
		<nav
			class="navbar align-items-center navbar-light w-100"
			ref="nav"
			v-bind:class="{
		...
		>
			...
			<span class="navbar-logo">
				...
				>
					<img
						class="menu-logo"
						v-bind:src="$helper.pathToUrl(model.logo)"
			...
			>
				<themeclean-components-textlinks
					v-bind:model="model"
				></themeclean-components-textlinks>
				<themeclean-components-menubuttons
					v-bind:model="model"
				></themeclean-components-menubuttons>
			</div>
	...
	export default {
		...
		methods: {
			onBlockMounted() {
				...
				if (
					this.model.sticky === "true" &&
					!$peregrineApp.isAuthorMode()
				) {
					if (this.$refs.section.style.position === "fixed") {
						const height = this.$refs.section.clientHeight;
						this.$refs.section.parentElement.style.marginTop =
							height + "px";
					}
				}
				const navSection =
					this.$refs.nav.parentElement.parentElement.parentElement;
				const navPosition = navSection.style.position;
				const navSticky =
					navPosition === "sticky" || navPosition === "fixed";
