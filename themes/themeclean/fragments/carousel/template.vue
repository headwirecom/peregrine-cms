<template>
	<themeclean-components-block v-bind:model="model">
		<div
			class="carousel slide w-100 text-light"
			ref="pcmscarousel"
			v-bind:style="`height:${model.carouselheight}vh;`"
			v-bind:id="name"
		>
			<ol class="carousel-indicators" v-if="model.indicators === 'true'">
				<li
					v-for="(item, i) in model.slides"
					:key="i"
			...
			<div role="listbox" class="carousel-inner">
				...
				>
					...
					<div
						class="percms-carousel-text p-3"
						v-if="item.heading || item.text"
						v-bind:class="{
							'percms-caption-bg': model.captionbg === 'true',
						}"
					>
			...
			$(this.$refs.pcmscarousel).carousel({
				...
				interval:
					this.model.autoplay === "true"
						? parseInt(this.model.interval) * 1000
						: 0,
