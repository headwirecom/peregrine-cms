<template>
	<section
		...
		ref="section"
		v-bind:class="[classes, colors]"
		v-bind:style="[styles, sticky]"
	...
	>
		...
		<div
			v-bind:class="
				model.fullwidth === 'true' ? 'container-fluid' : 'container'
			"
		>
...
<script>
	export default {
		...
		mounted() {
			...
			let navSection =
				document.querySelector("nav").parentElement.parentElement
					.parentElement;
		...
		computed: {
			classes: function () {
				let classObject = {};
				classObject["percms-view-height"] =
					this.model.fullheight == "true";
				classObject[`elevation-${this.model.elevation}`] =
					this.model.elevation > 0;
				return classObject;
			},
			colors: function () {
				let classes = {};
				...
				switch (this.model.backgroundtype) {
					case "image":
						...
						if (this.model.bgimage) {
							return (
								overlay +
								`url("${this.model.bgimage}") center center / cover no-repeat`
							);
						}
						return overlay;
