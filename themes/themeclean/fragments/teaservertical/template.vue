<template>
	<themeclean-components-block v-bind:model="model">
		<div class="col-12 col-md-10 align-items-center">
			...
			<div
				class="col-12 d-flex flex-wrap p-0 justify-content-center"
				v-if="model.showbutton == 'true'"
				v-bind:class="{
					'justify-content-md-start': model.alignbuttons === 'start',
					'justify-content-md-center':
						model.alignbuttons === 'center',
					'justify-content-md-end': model.alignbuttons === 'end',
				}"
			>
				<a
					class="btn m-2"
					v-for="(item, i) in model.buttons"
					:key="i"
					v-bind:href="$helper.pathToUrl(item.buttonlink)"
					v-bind:class="{
					...
					v-bind:style="`backgroundColor:${item.buttoncolor};borderColor:${item.buttoncolor};`"
				></a>
			</div>
