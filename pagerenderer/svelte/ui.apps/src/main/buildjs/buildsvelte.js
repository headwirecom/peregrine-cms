...
console.log('=== building svelte files ========================================')
...
function compileComponent(file){
    ...
    console.log("compiling svelte file: .%s", file)
  // get component name from file name (file: a.svelte, name: a)
  ...
  // compile the svelte component and give us a .js and .css
  rollup.rollup({
    input: `${basePath}${file}`,
    plugins: [
      svelte({
          dev: true,
          css: css => { css.write(`${distBasePath}/css/${nameCamelCase}.css`) }
        // compileTemplate: f,
...
// find all the svelte files in this project
var svelteFiles = readDirs(basePath, basePath, '.svelte')
var compiling = []
// for each of the files compile it
for(let i = 0; i < svelteFiles.length; i++) {
    compileComponent(svelteFiles[i])
}
