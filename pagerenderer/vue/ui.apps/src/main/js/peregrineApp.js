...
function getView() {
	try {
		if (
			window &&
			window.parent &&
			window.parent.$perAdminView &&
			window.parent.$perAdminView.pageView
		) {
			var mode = window.frameElement.attributes["data-per-mode"]
				? window.frameElement.attributes["data-per-mode"].value
				: null;
			if (mode === "tutorial") {
				return view;
			} else {
...
function processLoadedContent(data, path, firstTime, fromPopState) {
	data =
		window.$perProcessData !== undefined
			? window.$perProcessData(data)
			: data;
	...
	if (data.description)
		document.getElementsByTagName("meta").description.content =
			data.description;
	if (data.tags)
		document.getElementsByTagName("meta").keywords.content = data.tags.map(
			(tag) => tag.name
		);
	...
	processLoaders(data.loaders).then(() => {
		...
		if (document.location !== path && !fromPopState) {
			...
			document.title =
				getPerView().page.title + " | " + getPerView().page.brand;
			...
			if (domains) {
				for (var i = 0; i < domains.length; i++) {
					...
					if (
						...
						!(domain.match(/\w\/\w/) && domain.lastIndexOf("/") > 7)
					) {
						newLocation = "/" + path.split("/").slice(4).join("/");
						break;
					} else if (
						domain.match(/\w\/\w/) &&
						domain.lastIndexOf("/") > 7
					) {
						// domain contains a path
						newLocation = path.replace(
							/\/content(\/\w+)\/pages(\/.+)/,
							"$1$2"
						);
						break;
...
function loadContentImpl(path, firstTime, fromPopState, onPage = false) {
	...
	if (onPage) {
		processLoadedContent(
			JSON.parse(document.getElementById("perPage").innerHTML),
			path,
			firstTime,
			fromPopState
		);
...
function updateMeta(key, val, type) {
	var meta = document.querySelector(
		"meta[" + CSS.escape(type) + "=" + CSS.escape(key) + "]"
	);
